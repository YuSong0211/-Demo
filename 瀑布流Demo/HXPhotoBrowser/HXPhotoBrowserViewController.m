...
- (void)saveImage{
    ...
    UIImage *image = self.currentImageView.imageView.image;
    NSLog(@"=====%p",image);
...
- (void)setIndexLabel{
    if (!_indexLabel) {
        _indexLabel = [[UILabel alloc] initWithFrame:CGRectMake(0,  70, kHXSCREEN_WIDTH, 20)];
...
- (void)creatPhotoImageView{
    ...
    if (_currentIndex >= self.urlArray.count) {
        return;
    }
    if (self.isHasUrl) {
...
-(UIButton *)savebtn{
    if (!_savebtn) {
        ...
        [_savebtn setBackgroundImage:[UIImage imageNamed:@"chat_picimage_download"] forState:(UIControlStateNormal)];
        [_savebtn addTarget:self action:@selector(clickDownloadBtn) forControlEvents:UIControlEventTouchUpInside];
        _savebtn.frame = CGRectMake(kHXSCREEN_WIDTH - 50, 70, 32, 32);
    }
    return _savebtn;
}
